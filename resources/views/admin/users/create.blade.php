...
@section('pagetitle',__('admin.user_create'))
...
<form action="{{ route('admin.users.store') }}" method="POST">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
          <h6 class="m-0 font-weight-bold text-primary">{{ __('admin.user_create') }}</h6>
        </div>
        <div class="card-body">                
            <div class="form-row">
                <div class="form-group col">
                    <label for="formName">{{ __('admin.name') }}</label>
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" id="formName" aria-describedby="nameHelp">
                    {{-- <small id="nameHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> --}}
                </div>
                <div class="form-group col">
                    <label for="formEmail">{{ __('admin.user_email') }}</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror" id="formEmail" aria-describedby="emailHelp">
            ...
            <div class="form-row">
                <div class="form-group col">
                    ...
                    <input type="text" name="username" value="{{ old('username') }}" class="form-control @error('username') is-invalid @enderror" id="formUserName" aria-describedby="usernameHelp">
            ...
            <div class="form-row">
                <div class="form-group col">
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" class="form-control" id="formPhone" aria-describedby="phoneHelp">
                ...
                @foreach ($groups as $item)
                    <div class="col-md-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="groups[]" @if($item->id==4) checked @endif value="{{ $item->id }}" id="groupCheck{{ $item->id }}">
